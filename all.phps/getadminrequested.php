...
if($_SERVER['REQUEST_METHOD']=="POST" || $_SERVER['REQUEST_METHOD'] == "GET")
		{
			...
			FROM (SELECT complaintId, title, description, imagePath, IFNULL(ct.name, 'NA') AS name,
			 ...
			 FROM `customerComplaint` AS cc LEFT JOIN customerTable AS ct ON cc.customerId= ct.customerId LEFT JOIN vehicleDetails AS vd 
			 ON cc.vehicleId=vd.vehicleId WHERE approveStatus='pending') AS t1 LEFT JOIN vehicleModels AS vm ON t1.a= vm.vehicleTypeId";
			  }else{
			$sql="SELECT t1.*, IFNULL(vm.vehicleType, 'NA') AS vehicleType, IFNULL(vm.vehicleModel, 'NA') AS model 
			FROM (SELECT complaintId, title, description, imagePath, IFNULL(ct.name, 'NA') AS name,
			 IFNULL(ct.mobileNumber, 'NA') AS mobileNumber, cc.vehicleId, IFNULL(vd.vehicleTypeId, 'NA') AS a 
			 FROM `customerComplaint` AS cc LEFT JOIN customerTable AS ct ON cc.customerId= ct.customerId LEFT JOIN vehicleDetails AS vd 
			 ON cc.vehicleId=vd.vehicleId WHERE approveStatus='pending' AND cc.customerId='$customerId') AS t1 
			 LEFT JOIN vehicleModels AS vm ON t1.a= vm.vehicleTypeId";
		 }
